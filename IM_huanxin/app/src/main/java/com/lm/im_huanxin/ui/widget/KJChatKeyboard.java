...
import com.lm.im_huanxin.adapter.FaceCategroyAdapter;
import com.lm.im_huanxin.ui.fragment.OnOperationListener;
import com.lm.im_huanxin.ui.fragment.SoftKeyboardStateHelper;
...
public class KJChatKeyboard extends RelativeLayout implements
    ...
    private List<String> mFaceData=new ArrayList<String>();
    ...
    private void initWidget() {
        ...
        mEtMsg.addTextChangedListener(mTextWatcher);
    ...
    private TextWatcher mTextWatcher = new TextWatcher() {
        ...
        public void afterTextChanged(Editable s) {
            ...
            // 先去掉监听器，否则会出现栈溢出
            mEtMsg.removeTextChangedListener(mTextWatcher);

            // 注意这里只能每次都对整个EditText的内容求长度，不能对删除的单个字符求长度
            // 因为是中英文混合，单个字符而言，calculateLength函数都会返回1
            while (calculateLength(s.toString()) > 2000) { // 当输入字符个数超过限制的大小时，进行截断操作
                s.delete(editStart - 1, editEnd);
                editStart--;
                editEnd--;
            }
            mEtMsg.setSelection(editStart);
...
//        mFontSize.setText(getInputCount()+"/"+MAX_FONT_SIZE);
    }
    ...
    private long getInputCount() {
        return calculateLength(mEtMsg.getText().toString());
    }
    /*************************
     * 内部方法 start
     ************************/
